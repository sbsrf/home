...
重要概念：信息熵（疑义度）、KL散度（相对熵）、交叉熵、编码效率、冗余度（多余度） 、困惑度、传输效率、信道容量、信道利用率、有效信息速率、最大熵估计
...
- 基本指标：后面两小节会详细论述。
  ...
  - 高效：快速、准确
-  补充指标
   -  可用性：应用场景的多少，比如语音输入虽然简单、高效，但是在很多场合无法或者不便使用。 
   -  规范性：符合语文知识，与简单性重叠较多，因为有背景知识了。
   -  标准性：符合国家标准，少歧义，比如笔画标准禁止了倒笔画。令人尴尬的是，国家出台了双拼标准，而且也有了国标双拼方案，但是用户的采用率非常低。国家也出台了部件标准，但是还没看到一个完全采这个部件标准的输入方案出现。
...
图的标注问题：不影响信道容量，但影响信道的利用率。成形编码的目的就是提高信道利用率。
...
其中，$N(T)$ 是 $T$ 时间内容许的信号数量。通过大量的用户操作实验，自然可以得到一个平均击键速度的统计值，用以当作键盘信道的近似值，但是非常耗时耗力，且不太准确。在理论上，则可以建立离散信道的数学模型来精确计算键盘的信道容量，从简到繁分的模型为：等时长模型、不等时长模型、状态转移模型、击键当量模型。
...
例如，假设用于输入的按键是标准QWERTY键盘的26个字母，如果每个按键平均需要0.1秒（即每秒10键），那么信道容量约为 $10 \times \log_2 26 \approx 47$ 比特/秒。

同理，电传打字机来的键盘信道容量的计算非常简单，因为每次按键的时间是固定的和一样的，就可以采用这个模型。假设键盘有 32 个键可用，那么每个按键就代表了 $log_2{32}=5$ 比特的信息。如果每秒钟按键 n 次，该信道每秒钟就可传输 5n 比特的信息。
...
这相当于在平均时间约束下最大化信息传输速率。莫尔斯电报机的信道容量计算就适合采用这个模型，可以用以下行列式计算。
$$
\left| \begin{array}{cc} 
...
更精确的模型需要考虑手指的当前位置和状态，因为从一个键移动到另一个键的时间取决于两个键的相对位置。
...
不同的输入方案对反馈的依赖程度差异很大，依赖越小则可盲打性越高。大致可以分为以下几类：
...
从信息论的角度看，对反馈的依赖程度本质上反映了**正向信道的信息传递效率**。正向信道传递的信息量越大（编码越长、重码越少），所需的反馈就越少；反之，正向信道越模糊（编码越短、重码越多），就越依赖反馈来消除歧义。
