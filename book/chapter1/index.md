...
本书将输入法定义为将文本输入到智能设备的方法，其具体实现形式为人机交互的输入系统。

智能设备可以是计算机、手机、平板电脑、智慧屏等，它们的特点是具有操作系统和强大的解码能力，使得输入法的实现成为可能。

文本可以是中文的、英文的或者其它文字的。对于字母文字而言，输入法与机械打字机几乎一样，只是字母向按键的简单映射。但是，对于以中文为代表的表意文字而言，情况就大不一样了。如何用几十个按键输入几万个汉字，涉及到的问题纷繁复杂，解决起来也是困难重重。

一般来说，输入法是给人使用的，通过键盘进行的，而且是操作密集型的。但是，在有的情况下，人参与的程度很低，而且不一定使用键盘。例如，在批量语音识别和批量字形识别时，人只是操作输入系统，而不需要参与输入过程。再如，用模拟打字软件自动打字时，人也不参与具体的输入过程。

本书主要论述中文输入法。严格来说现代中文还包括标点符号和阿拉伯数字等非中文符号，但由于中文输入的主体和难点在于汉字的输入，所以也常常称为汉字输入法，或者干脆简称为输入法。
...
- 在理论上缺乏严密的基础，不严肃的民科思想泛滥，盲目浮夸吹嘘之风盛行。
- 学术上完全无视编码在输入法中的作用，仅仅试图在解码环节来解决问题。
- 大厂输入法对顶功技术和开源 Rime 忽视、无知或抗拒，同质化非常严重。

## 1.4 解决的方法

- 强化理论基础，建立严格的输入法理论框架。
- 强化学术研究，关注输入法的理论基础和实际应用。
- 保护开源 Rime 等输入法的版权，防止被大厂输入法所占。
