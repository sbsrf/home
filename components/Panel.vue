...
import LibrimeWorker from "../src/util";

const worker = inject<Ref<LibrimeWorker>>(workerKey)!;
const text = inject<Ref<string>>(textKey)!;
...
async function analyze(result: any, rimeKey: string) {
  const textarea = getTextarea();
...
async function input(rimeKey: string) {
  const result = await worker.value!.process(rimeKey);
...
function onKeydown(e: KeyboardEvent) {
  ...
  const isShortcut =
    hasControl || hasMeta || hasAlt || (hasShift && !isPrintableKey);
  ...
  if (!editing.value) {
    ...
    if (
      isShortcut &&
      !hasShift &&
      !(hasControl && CONTROL_ALLOWLIST.includes(key))
    ) {
      return;
    }
  }
...
async function onClick(key: number) {
  const result = JSON.parse(await worker.value!.selectCandidateOnCurrentPage(key));
...
<template>
  ...
  >
    ...
    <n-button text :disabled="nextDisabled">
      <n-icon :component="CaretRight" @click="input('=')" />
    </n-button>
  </n-popover>
</template>
