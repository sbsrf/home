...
根据不同的应用场景，分词的颗粒度可细可粗，以便在低打空率和高压缩率之间取得最佳的平衡。若以单字为单位来输入，则肯定不会打空，打空率为零，而且不需要分词，分词成本也为零。然而，单字用键多，重码也多，输入效率太低。

现代汉语是以二字词为主导的，三字词和多字词也日益增多。在词库增大到100万时情况就翻转了，四字词多于三字词，而三字词又多于二字词。声笔简拼收词量高达200万，所以这种情况就更加突出。

声笔简拼的单字输入是其弱项，更应该尽量以词为单位来进行输入，而且分词的颗粒度在可控的打空率下应当尽量地以较粗为好，因为声笔简拼具有自动造词的能力，三字词和多字词的空间巨大，打空也是一次性的，组词成功后，就可以当成正常词组来使用了。
...
1. 单字：`sb*8`，即声母加最多八个笔画，最长编码空间为`21*5^8 = 8,203,125`；
2. 二字词：`s(b*8)sbb`，即各字的声母加末字的前两笔，再加首字的前八笔，最长编码空间为`21^2*5^10 = 4,306,640,625`；
3. 三字词：`s(bb)s(bb)sbb`，即各字的声母加末字的前两笔，加首字的前两笔，再加第二字的前两笔，最长编码空间为`21^3*5^6 = 144,703,125`；
4. 四字词：`s(bb)s(bb)s(bb)sbb`，即各字的声母加末字的前两笔，加首字的前两笔，加次字的前两笔，再加第三字的前两笔，最长编码空间为`21^4*5^8 = 75,969,140,625`；
5. 多字词（五字及以上的词）：`s(bb)s(bb)s(bb)sS*bb`，编码格式与四字词类似，只是声母从第五字起需要用大写，第四字起不再补充笔画，五字词的最长编码空间为`21^5*5^8 = 1,595,351,953,125`。
...
对于单字来说，输入时码长可能为1至9码，比如「一y 二ve 笔bue 型xeeu 戴deieia 麟loeuai」。注意，从第三码起，码位上的字是动态变化的，用户越常用的字码长会变得越短。

对于二字词来说，输入时码长可能为2至12码，其中首字可长达9码，末字3码，比如「但是ds 了解lju 理解ljua 两句lejua 两脚leijua 落脚leiijua 菱角leiiejua 联句leiieejua」。注意，二字词的码长也是从第三码开始调整的，所以你输入时的结果可能会不一样。特别地，补充首字笔画的时候，并不需要用户先定位到第二字的声母前，而是光标仍然在整个编码的末尾，补充的笔画会自动插入到第二字编码的末尾作为编码。

对于三字词来说，输入时码长可能为4至9码，每字最多3码，比如「计算机jsje 接收机jsjei 净水机josjei 减速机joesjei 浆纱机joesajei」。

对于四字词来说，输入时码长可能为4至12码，每字最多3码，比如「今生今世jsjs 驾驶技术jsjse 计算技术jsjsei 军事技术josjsei 计算加速joasjsei」。
...
为了尽量缩短码长，就需要优先使用超级简拼，但是超级简拼的空间非常有限。在声笔简拼中，单字的超级简拼空间只有21个，二字词的超级简拼空间也只有441个，面对数万个单字和数十万个二字词，势必会产生大量的重码。三字词和多字词的超级简拼情况重码的情况也非常严重。

处理重码最简单的办法就是列出来让用户选择，然而这是极其低效的方法，因为涉及到大量的扫视和翻页操作。高效的办法是加码筛选，就像查字典时使用索引一样。加什么样的码呢？对于传统拼音输入法如搜狗拼音来说，就是加末字的韵母，而对于声笔简拼来说就是加末字的笔画。当然，就单字来说，末字就是单字本身。

搜狗拼音如果强制使用超级简拼的话，因为没有快捷的补码手段，输入时会遇到巨大的困难。繁琐的回改将使得超级简拼节约的用键变得无足轻重，往往得不偿失。所以，在搜狗拼音中，为了保险起见，除了超高频的词，一般是先用全拼输入一次以后，再使用超级简拼，不但降低了输入效率，而且还失去了输入的一致性。另外，由于`a e o`可以出现在超级简拼中，而`n g`可以被解释为韵母的一部分，因此会导致超级简拼的解释出现歧义，降低了音字转换的准确性。
...
补码解决了快速筛选重码的问题，但却没能解决很多字词的码长太长的问题。在声笔简拼中，单字和二字词的短码空间很小，码长问题显得特别突出。系统默认的字频和词频，对于一些普通常用字词，可能适合大多数用户。然而，每个用户都肯定会有自己的一套字词使用习惯，希望自己用得多的字词具有更短的码长。实时动态调频就可以做到这一点。

比如，对于学习和研究声笔输入法的人来说，「声笔」和「声笔简拼」都是高频词，而对于其他人来说它们可能就是低频词，甚至根本就不是词。通过调频，「声笔」只需要三键就可以输入，而「声笔简拼」只要四键就可以输入。在频繁使用的情况下，节约的用键是极其可观的。
...
例如，在输入dei后，如果首选是想要的「都」字，那么在有后续输入的情况下，是不必用空格上屏的，只需要输入后续字词的首码，「都」就会被顶上屏幕。
...
打空以后，声笔简拼可以无缝地进行组词。办法是，先顺序加末字两码，然后补首字、次字等的编码，在加码的过程中观察是否出现所需的词组。当候选中出现比所需词组更短的词时，说明系统没有所需词组。此时，若出现构成所需词组前缀的字词，则选中进行组词。对于三字词和多字词，在补码输入完每字三码后，系统会禁止继续补码，以免发生错误。在必要的时候，也可以手动定位进行干预。重复以上过程，直至造好所需词组。
...
除了一、二简字词外，声笔简拼还有大量固定的其它简码字词，包括数选字词以及少量标点字词。其中，简码字有1512个，包括210个声数字、1050个声笔数字、42个声标字、210个声笔标字，而数选简码词有4410个。简码字词有助于提高声笔简拼的可盲打性，从而提高输入效率。
