...
除了一、二简字词外，声笔简拼还有大量固定的数选字词，包括210个声数字、1050个声笔数字和4410个声声数词，它们有助于提高声笔简拼的可盲打性，从而提高输入效率。每种数选字词均等分为有理的和无理的两组，默认情况下只提示有理组，可以设置为同时提示无理组，以下示例是设置为同时提示无理组的情况。有理组用23789代表折横撇竖捺，而无理组用`’456`和分号或者制表符进行选择，与笔画无关。

例如，在输入z后，如果直接输入`’456;`将顺序选择「再 之 作 主 真」五个字。

![image-20240606065458208](/images/image-20240606065458208.png)

再如，输入ze后的情况如下，选择方法与上例相同，输入`’456;`将顺序选择「招 直 政 至 找」五个字。

![image-20240606072505577](/images/image-20240606072505577.png)

又如，在输入zy后，如果直接输入`’456`或者Tab将顺序选择「怎样 重要 只要 主要 终于」五个词。

![image-20240519080226329](/images/image-20240519080226329.png)

声笔简拼中的数选字词和声笔简码中的是完全一样的，只是声笔简拼用单引号代替了1，而用分号或者制表符代替了0，因为它将1和0用来定位了，无法使用数字1和0来选择。
