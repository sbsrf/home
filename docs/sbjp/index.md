...
1. 单字：`sb*5`，即声母加最多八个笔画，编码空间为`21*5^8 = 8,203,125`。
2. 词组：`s(b*8)s*2S*sb*2`，即各字的声母（从第五字起声母需要用大写），加末字的前两笔，再加首字的最多前八笔，编码空间为`21^2*5^10 = 4,306,640,625`，三字词增加21倍达到`90,439,453,125`，四字词再增加21倍达到`1,899,228,515,625`，等等。
...
对于三字词来说，输入时码长可能为4至13码，其中首字可长达9码，第二字1码，第三字3码，比如「计算机jsje 接收机jsjei 净水机josjei 减速机joesjei 浆纱机joeisjei」。

对于四字词来说，输入时码长可能为4至14码，其中首可长达9码，第二、三字1码，第四字3码，比如「今生今世jsjs 驾驶技术jsjse 计算技术jsjsei 军事技术josjsei 计算加速joasjsei」。
...
打空以后，声笔简拼可以无缝地进行造词。办法是，先顺序补末字两码，然后补首字的编码，在补码的过程中观察是否出现所需的词组。当候选中出现比所需词组更短的词时，说明系统没有所需词组。此时，若出现构成所需词组前缀的字词，则选中进行组词。在补码输入完首字的九码后，系统会禁止继续补码，以免发生错误。在必要的时候，也可以手动定位进行干预。重复以上过程，直至造好所需词组。
