...
如果当前处于中文状态，且已经输入有编码，则可采用以下定位方法：
...
如果当前处于中文状态，已经打开auto_inline选项（默认未开启），且还未输入任何编码，那么首字母大写就会自动进入临时英文状态。
...
如果当前处于中文状态，且已经输入有编码，若要上屏首选项并切换为英文状态，那么在电脑上单按右Shift即可,在手机上则上滑空格。
...
如果当前处于中文状态，且已经输入有编码，若要上屏编码（字母）并切换到英文状态，那么在电脑上可单按左`Control`键，在手机上还无法实现。

如果当前处于中文状态，要锁定大写来输入英文，那么在电脑上单按`CAPSLOCK`即可，在手机上则长按Shift来实现。

如果当前处于中文状态，且已经输入有编码，若要清除已经输入的编码，在电脑和手机上都可单按`Esc`键，在手机上还可左滑退格键，在电脑上还可用Shift+退格键。
...
通过`\`引导，可以输入各类特殊符号，支持用`aeuio`选择重码。重码超过一页时。可用Tab翻页查找。

![image-20230903081759352](/images/image-20230903081759352.png)
...
不少用户需要的一些特殊词组是方案本身不能或者不便于造出来的，比如带数字、符号和字母的词组。声笔系列码提供了一个各输入方案共享的自定义词典来满足这种需求，文件名为sbzdy，意为声笔自定义。

由于声笔系列码在不少情况下是不允许重码的，所以用户自定义的词组一般不能放在系统词组的编码空间里，否则可能造成冲突而影响输入法的正常使用。经过仔细考量，决定采用`u`引导来解决用户自定义词组的输入问题，此处的`u`意为「用户」，因为它是英语的`user`的首字母，而声笔系列码的各种方案都不会使用`u`作为正常中文编码的首码，不会产生编码冲突。

![image-20230705150553016](/images/image-20230705150553016.png)

上图所示就是自定义词典文件。在该文件开始的注释部分，说明了编码格式和注意事项。需要特别注意的是，自定义词组不能以`aeuio`作为首码，非首码可以为非aeuio的小写字母和数字，这样可以保证能用aeuio选择重码，否则重码字词需要通过移动光标来选择，很不方便。重码时，可通过权重来控制顺序，权重越大越靠前。同时，还要强调，词组和编码之间，编码和权重（如果有的话）之间的分隔符必须是制表符TAB，不能是空格，否则会导致自定义词典编译失败，所以要找一个支持制表符的编辑器来编辑自定义词典。有的编辑器如Sublime默认时会将TAB转换为几个空格，需要修改它的设置才能保留TAB。

### 6.2 可定制专用词典

声笔收录词组的数量高达48万多条，默认是进行动态调频的，以便在使用的过程中自动地调教词库，不断适配用户的打字习惯。但是，在遇到重码词都很常用时，就会发生候选位置频繁变化的问题，大大影响打字的确定性。另外，个别字词的编码打起来可能很别手。怎么办呢？利用可定制专用词典就能解决问题。

以下列出专用的可定制词典，说明其适用的方案。专用可定制词典的具体用法，请参见各输入方案的教程。
...
声笔程序默认自带了多种LUA脚本。通过LUA脚本能够实现的功能是非常多的，网上也有不少现成的脚本可以下载，请自行添加。有能力的用户也可自己动手编写脚本来实现特定的功能。

### 7.1 o引导的脚本

用`orq`来输入日期，用`osj`来输入时间，用`o[0-9]+`来输入中文数字。

![image-20221125100100623](/images/image-20221125100100623.png)

![image-20221125100122408](/images/image-20221125100122408.png)

![image-20221125100151474](/images/image-20221125100151474.png)

### 7.2 e引导的脚本

用e加表达式来完成各种计算功能，相当于一个强大的快速计算器，例子如下。
...
![image-20230817161333742](/images/image-20230817161333742.png)
...
声笔系列的所有输入方案默认都是开启自动调频与自动造词的，这适合大多数人的习惯。但是，仍然有些用户不喜欢词库的动态变化，或者喜欢在一段时间的动态调整后就不再变化。为此，声笔定制程序专门针对Table Translator提供了一个stop_change选项，用于停止或者开启自动调频与自动造词，停止为true，开启为false。用户修改schema中的这个选项后，需要重新部署才能生效。该选项的位置如下图所示。该选项也能用于非声笔系列的方案。
...
声笔系列的所有字词方案默认都能自动删除过时的临时词组。默认的删除规则是，若在输入时检索到某个临时词组的tick值与最新的全局tick值之差的绝对值大于等于5000时，则将该临时词组删除。这个绝对值称为delete_threshold，可以在Translator中修改。当它小于等于0时，则不会删除临时词组，否则就按该值规定的tick差值来删除。该值的通俗含义就是，检索到某个临时词组前你一共输入了多少个字词。

### 8.3 自动造词长度

在默认情况下，自动组词的长度限制为5个汉字，用户可以通过修改方案的schema改变此值，但是一般不建议修改此值，因为中文里超过5个汉字的词是很少的。在确实需要超过5个汉字的长词时，可以通过用首码后用`Shift + Tab`切换为缓冲模式来创建。在长词上屏后，会自动退出缓冲模式。缓冲模式下造词的长度为12个汉字。超过12个汉字的长词，可以在sbzdy.dict中定义。如果想停止自动造词而仅仅保留缓冲造词，那么可以保持可以将schema中translator里的max_phrase_length设置为1。
...
> 我们他们什么自己知道可以这个这样已经发展因为现在怎么你们工作问题经济如果只是这些企业时候所以一定这里并不关系进行可能但是今天地方觉得建设时间虽然一样姑娘第一女人一些如何突然之后告诉记者不要市场开始看到爱人同时东西政府系统发现希望孩子一切一直身上认为十分先生管理原来必须不住那些表示应该无法忽然许多情况需要因此改革事情国际相信得到朋友喜欢感到第二北京出现完全通过发生离开美国其中领导组织实现活动根本一般特别服务于是教育问道非常方法父亲以后提出目前几乎代表文化其他会议清楚身子小姐加强听到人们老人难道作为建立无关结果今年环境处理人员似的不错科技女子其实历史能够电话立即认识工程地区参加不断女儿同志支持众人准备那样努力网络条件口气基础点头此时香港罢工仍然产品身边如今微笑进入存在才能微微两国果然奇怪理论衣服轻轻控制只能马上基本这儿单位科学和尚之前取得后来成功名字日本保护冷笑完成缓缓少女敌人此刻目标关于身体项目表情农村不肯办法法律大哥水平形成痛苦摇头本来共同后面开放露出房间弟子年轻总统青年而已安危厉害恢复充满认真感情显然另外采取目的慢慢心情考虑发挥一块耳边打开恐怕唯一统一传统反而毫无很快台湾连忙产业美丽功夫带来美元模型领域太太充分只怕调查女孩父母多年讨论情形人才反应那时温柔依然人类按照发表承认文件年纪法国麻烦客人害怕漂亮多么幸福强烈一旁或许旁边加快安排革命反对所谓出版片刻期间程度宣布位置妇女平静召开无比澳门妇人毫不罢免体系可爱黑暗常常确保男女千万爱情离去热情老板谈话训练无奈从未时刻普通老者因而多久急忙极为必然完善老头暗藏顺利提起破坏好奇名称面积把握规划放开儿童气氛谈判念头案件耳朵热闹会谈何处美人各位恶心脾气胸口培训分配批准赶快拼命两岸眉头老婆客厅认得模糊偏偏送给平常解放恐怖考察回归昂首平时性能情人佩服客户判断半天大批前后渴望浪费美女头脑能否仆人骄傲以免金牌半年模块难看窗户颇为而后偶尔儿女喜爱长安屁股客观河南强迫案犯可靠发帖门派热线无派股票理念云南签名宁波芯片案例我爱是啊备案那位昆明配偶认可图案热爱认同哪怕网卡被迫乳房美味爱国突发关爱陪你请按名额软盘安康
