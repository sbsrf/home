...
1. 词组全码：`26^4*5 = 2,284,880`，可以轻松容纳巨型词库
2. 词组简码
   - 三码简词：`26*26*5 = 3380`，简词独享
   - 四码简词：`26^3*5 = 87,880`，与单字共享
...
编码模式是中文键盘输入的重中之重。设计猛码的首要目的也是对编码模式进行创新。顶功是编码模式中的一大类，影响最大的是二顶，其次是四二顶和五二顶。传统上，定长一直占据着编码模式中的主流地位，最常见的是四定，其次是五定和三定。对定长方案的改进，一般采用简码多重的方法。整句也是比较传统的编码模式。前缀则是最近复兴的编码模式，但它实际上出现得很早，90年代初出现的文字码就已经采用了前缀码的方式。

## 7 测试样例

以下为用大竹作为测试工具进行的一些测试样例，与同为乱序但为四定方案的虎码和同为乱序但为三定的三奕进行对比，以便直观演示一下猛码新型顶功模式的效果，不过并非全面的测试结果。
