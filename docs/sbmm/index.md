...
3. 多根字：Z¹Z²Z³F⁰，即首根大码 + 次根大码 + 三根小码 + 末根首笔。例如，在「输 nsh;」中，首码n为首根「车」的乱序大码，第二码s为次根「人」的乱序大码，第三码h为三根「一」的乱序大码，末码为末根「刂」的首笔竖。
...
1. 一简字：全码的首码加空格，例如「在d 人s 他u 不o 以m 到l」。
2. 二简字：两码字一般通过延迟顶上屏而省略空格，当然也可空格上屏。不过，要注意的是因为`,;/.'`作为猛码二简词的第三码了，所以不能用这几个标点来顶屏。
...
- 二字词：各字前两码加末字末根首笔的数字形式。
- 三字词：前两字的首码加末字前两码再加末字末根首笔的数字形式。
- 多字词：前三末一字的首码加末字末根首笔的数字形式。
...
![image-20251123115748913](/images/image-20251123115748913.png)
...
猛码在默认情况下是采用的主单模式，即以单字为主进行输入的模式，虽然猛码在主单模式下的词组能力也是相当强大的。上文所讲述的都是单字模式的内容，接下来会讲主词模式。
...
编码模式是中文键盘输入的重中之重。设计猛码的首要目的也是对编码模式进行创新。顶功是编码模式中的一大类，影响最大的是二顶，其次是四二顶和五二顶。传统上，定长一直占据着编码模式中的主流地位，最常见的是四定，其次是五定和三定。对定长方案的改进，传统上一般采用简码多重的方法。整句也是比较传统的编码模式。前缀则是最近复兴的编码模式，但它实际上出现得很早。

## 8 测试样例

以下为用大竹作为测试工具进行的一些测试样例，与同为乱序但为四定方案的虎码和同为乱序但为三定的三奕进行对比，以便说明猛码新型顶功模式的效果。
