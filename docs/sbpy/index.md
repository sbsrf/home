...
7. 补笔画：补充笔画直至码长为6，笔画不够时重复末笔；横竖撇捺折五个笔画用`eiuoa`来表示。例如，「笔」的拼音为bi，追加前3个笔画后编码为`biueou`。又如，「将」的音节为`jaa`，追加前两个笔画后的编码为`jaaoei`。再如，「乙」的音节为`yi`，而「乙」只有一个笔画，所以要重复三次才能形成编码`yiaaaa`。需要注意的是，在实际输入的时候，一般是不用6位全码的。
...
然而，如果要输入的是「建设奖」，输入到jsjaaoe后在当页候选中仍然没有。这时，发现前两字是一个常见的二字词，于是按两次1或者一次单引号定位到前两字后，准备进行修改。
...
另外，还有7个特殊的二简字，它们是利用正常编码留下的空码安排的特殊字，只有声母是有理的，主要用来缩短码长。这7个字是：`幅fi 贵gi 虎hi 跨ki 嚅ri 哎vi 唯wi 矮vu`。

#### 3.2.2 二简词

在声笔拼音中，取两字声母作为编码的高频二字词称为二简词，例如：`现在xz 但是ds 建立jl`。这些二简词一共有下441个，具体参见声笔基础一章中的[声声词](../sbjc/#_9-4-3-声声词)。
...
一级数选字有下列105个，在输入声母后会进行提示。这105个是有理的，用23789选择，分别表示其首笔是折横竖撇捺，与声笔简码中用aeuio表示的声笔字完全相同，见声笔基础一章中的[声笔字](../sbjc/#_9-4-2-声笔字)。

![image-20240518074938704](/images/image-20240518074938704.png)

二级数选字有下列489个，均为无理选择，这里的23789不表示笔画，在后面有些非常用字，在输入前两码后会进行提示。

![image-20240518075002080](/images/image-20240518075002080.png)

另外，还有下列80个特殊的二级数选字，分为8组，它们是利用正常编码留下的空码安排的特殊字，用于缩短码长，第二码是首笔画。

![image-20240705085916227](/images/image-20240705085916227.png)

#### 3.3.2 数选词

声笔拼音的数选词采用两字的声母，再用数字来选择输入，在输入两码后会进行提示，数量较多，有2205个。其中，有2205个为有理的，数字选择键`23789`与`aeuio`一一对应，代表第二字首笔画。`456`保留供自定义，1和0是用来定位的，所以没有安排词组。

![image-20240518075053256](/images/image-20240518075053256.png)
...
如果将schema中translator项下的fix_combination子项设为true，则在输入三个声母时，在首选项为二简字和一简字的组合，绝大多数不是真正词，例如：`应该的ygd 知道了zdl`，可以用空格上屏。若是一简字加二简字的组合，则通过加分号变换来输入，例如：`的时候dsh; 不一样byy;`。这一点和传统拼音输入法是大不相同的，其好处是一方面可以利用三码空间来安排固定组合，另一方面又不会增加三字词的码长，却增加了输入的确定性。
...
声笔拼音还有大量扩展的数选字词。这些字词是对第3节固定字词的进一步增强。扩展数选字有105个sx字和485个syx字，其中s表示声母，y表示韵母的首字母，x表示`’456;`，是无理选择。扩展数选词有2205个，其格式为ssx，其中ss表示两字的声母，x表示`’456`或者Tab，也是无理选择。在设置为同时提示无理组时，就可以看到扩展数选字词的提示了。

例如，在输入z后，如果直接输入`’456;`将顺序选择「指 资 字 坐 掌」五个字。

![image-20240511113649101](/images/image-20240511113649101.png)

再如，输入ze后的情况如下，选择方法与上例相同。

![image-20240511113735600](/images/image-20240511113735600.png)

又如，在输入zy后，如果直接输入`’456`或者Tab将顺序选择「怎样 重要 只要 主要 终于」五个词。

![image-20240518075208648](/images/image-20240518075208648.png)

### 3.7 自定词典

声笔拼音的所有固定字词，包括一简字、声韵字、声声词等，以及所有固定的数选字词，都是可以修改定制的，它们位于sbpy.fixed.txt文件中。您修改保存后，重新部署一下就可以生效。
...
在表达式中，对于每个字，s表示声母即首码，y表示韵母中的第一个字母（即次码），x表示韵母中的第二个字母或者追加的第一个笔画（即第三码），大写的S表示大写声母，`'`表示起始调频位，`^`表示起始顶屏位，`"`表示回头补码位，小括号中的编码为可选的。在有两种格式时，在括号中注明了混顶或纯顶的则只能用于混顶或纯顶模式，否则混顶模式和纯顶模式下均可用。
...
![image-20240518075305200](/images/image-20240518075305200.png)

#### 4.1.2 实例二

「感」是二简字，只需要声母g和韵母中的第一个字母a（相当于输入格式中的sy）即可。如果后续其它字词如「性」的声母x，那么在纯顶时，「感」就会被顶上屏幕，从而节约空格键，而在混顶时，则必须用空格上屏后才能接着输入「性」字。

![image-20240518075332368](/images/image-20240518075332368.png)
...
![image-20240518075809665](/images/image-20240518075809665.png)

![image-20240518075830459](/images/image-20240518075830459.png)

![image-20240518075846003](/images/image-20240518075846003.png)
...
![image-20240518080006848](/images/image-20240518080006848.png)

也可以追加一码，出现在首选了，可以顶屏。 追加两码，仍然在首选，仍然可以顶屏。 

![image-20240518080035345](/images/image-20240518080035345.png)

![image-20240518080052025](/images/image-20240518080052025.png)
...
![image-20240518080147369](/images/image-20240518080147369.png)

追加一码，该词出现了，可以用3选择上屏。  

![image-20240518080210113](/images/image-20240518080210113.png)

如果追加为gaaxio，该词成为首选了，可以顶屏。  

![image-20240518080240537](/images/image-20240518080240537.png)
...
![image-20240518080352457](/images/image-20240518080352457.png)

#### 4.3.2 实例二

「脚手架」一词，输入jsja后，并没有出现在首选，可以数选上屏，也可以继续输入末字的编码，该词就出现在候选项的首位了，可以被顶。

![image-20240518080746737](/images/image-20240518080746737.png)

![image-20240518080809442](/images/image-20240518080809442.png)
...
由于候选项中没有「候选项」，于是回头追加首字的编码。

![image-20240518085042236](/images/image-20240518085042236.png)

发现已经没有三字词，开始出现二字词了，于是进行组词，按单引号定位到第二字后。

![image-20240518085144308](/images/image-20240518085144308.png)

第4项就是所需的「击键」，选中它。   

![image-20240518085228051](/images/image-20240518085228051.png)

第4项正好是所需的「率」，选中它就完成了输入过程。  
...
也可以，回头补充首字编码，直到「永生永世」便成为首选，以便顶屏。
...
由于候选项还是没有变化，所以再回头补充首字的编码。

![image-20240518090757150](/images/image-20240518090757150.png)
...
![image-20240518091014967](/images/image-20240518091014967.png)
...
![image-20240518091312190](/images/image-20240518091312190.png)
...
![image-20240518091206478](/images/image-20240518091206478.png) 

#### 4.5.3 实例三  

在纯顶模式下，假设现在要输入「声笔系列码」，发现在输入sbxlim后，情况如下。

![image-20240518091451342](/images/image-20240518091451342.png)

由于之前输入过「声笔系列」，它在候选项中出现了，选择后情况如下。

![image-20240518091526862](/images/image-20240518091526862.png)

继续追加「码」字的两个编码，「码」字即出现在首选了。

![image-20240518091556046](/images/image-20240518091556046.png)
...
![image-20240518091947152](/images/image-20240518091947152.png)

没有该词，且候选项无用，于是继续追加末字两码。

![image-20240518092042212](/images/image-20240518092042212.png)

情况没有改观，再回头补充首字编码。

![image-20240518092145606](/images/image-20240518092145606.png)

「顶功」出现了，选择上屏。

![image-20240518092234968](/images/image-20240518092234968.png)

「输入」还未出现在候选项中，再回头补充「输」的编码，直到它出现。

![image-20240518092314606](/images/image-20240518092314606.png)

选择「输入」上屏后，「高手」也出现在候选项中了。

![image-20240518092607247](/images/image-20240518092607247.png)

整个造词过程犹如行云流水，与常规的输入过程几乎是合而为一了。  
