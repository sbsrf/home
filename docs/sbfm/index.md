...
无论对单字派还是词组派来说，飞码在码长和击键方面都优于五笔，从而具有更快的打字速度，因为打字速度=击键×60÷码长。即使在主词模式的时候，飞码的三简字仍然可以省略空格而比五笔短，比五笔缩短了12.41% = (3.26 - 2.90) /2.90 * 100%，而且飞码拥有更强大的词组能力，可以使用更多的词组，从而进一步缩短码长。另外，飞码还可以通过组合变换来进一步省略用键，从而提高效率。

而且，飞码比五笔的习得成本更低，花费的学习时间要少得多。如果从声笔的其它方案转学飞码则学习曲线会更加平滑。

由于飞码的单字编码与飞单完全相同，同时对于单字派来说，可能只关心和使用飞单，所以飞单是独立成章进行讲解的。若您是初学者，请务必先了解和掌握[声笔飞单](../sbfd/)，然后再回过头来继续学习飞码。
...
声笔字的组合，以及二简字与一简字的组合，不管是否构成词组，是绝对没有重码的，可以放心大胆地盲打。声笔字与声笔字组合的例子有`大家dejo 前进qoje`。二简字与一简字组合的例子有`非就fij 还是hes 放可ffk 都是dvs`。
...
大量一、二简字的组合，不管是不是词组，都可以通过组合变换来进行输入。注意，这个时候是没有提示的，需要用户对简字非常熟习才能灵活自如。编码变换一般通过分号键触发，有时也通过Tab键和单引号触发。
...
> 输入法srfd 对于dayv 任何rrhr 学习xxxd 和h`_`使用 sryu 中文ziwd 的人`dr;` 都是dvs`_` 极其jmqh 重要zvyx 的d_ 。.

可见，除了「和」与最后的「的」这两个一简字外都没有用到空格，「的人」是通过组合来节约空格的。而且，在二字词中的三简字和全码字也只用了两键，「输入法」这个多字词一共才用了四键，输入效率的提升还是比较明显的。当然，在遇到非首选字词时，还是需要选重的。
...
如果是在飞码的单字状态（即飞单）下，则二简字也是可以被顶屏的。比如，「入」字，在飞码和五笔中的编码分别为rp和ty，都为两码。在单字状态下，接着打「口」字时，飞码可以用「口」的第一码顶出「入」而省略空格，而五笔则必须先用空格上屏后，才能打「口」的第一码。
...
为了缩短词组码长和增加输入确定性，提高输入效率，特为飞码提供了用数字选择的字词，称为数选字词，可以在增强模式下使用。
...
例如，在输入「不伤心」时，通常得用`b_sx9`，但是由于有了延迟顶屏，就只用 `bsx9` 就可以输入了。
