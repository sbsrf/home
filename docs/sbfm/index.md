...
除了词组模式和单字模式外，飞码还提供了延顶模式。可以在有2位编码时按F6或Shift+空格快速在延顶、单字和词组模式之间切换。如果要设置延顶为默认模式，那么就需要修改sbfm.schema.yaml，并重新部署。

延顶模式也是一种以单字为主的打字模式，它在兼容飞单的基础上，通过延迟顶屏克服了单字模式存在的一些问题，使得在输入词组时更加自然，具有词组模式下的组合变换能力，能够过滤掉低效的二字词。

比如，在输入编码dbzg的时候，结果如下图所示。

![image-20250123091055898](/images/image-20250123091055898.png)

继续输入编码会顶「点」字上屏，并留下zg和刚输入的编码。用空格可上屏两个二简字的组合即「点张」，用单引号可上屏二简字和二简词的组合即「点这个」，用Shift+空格可上屏二简字和两个一简字的组合即「点在个」。如果用大写AEUIO，则选中相应的候选项，但不会直接上屏，以便输入扩展编码。

要想直接上屏次选的「单字」，则可用分号；若要直接上屏第三候选项，则可用Tab键。注意，这时不能像词组模式那样在四码时用分号来上屏两个二简字和用Tab来翻页，只能通过扩展编码来输入非首页的重码词，或者可以通过直接用`PageDown`（手机上是下滑Tab）来向下翻页。`Shift+Tab`（手机上是上滑Tab）的向上翻页功能仍然有效，仍可用于定位到最后一页来快速找到新造的词组。
