...
延顶模式也是一种以单字为主的打字模式，它在兼容飞单的基础上，通过延迟顶屏克服了单字模式存在的一些问题，使得在输入词组时更加自然，拥有比词组模式更强的组合变换能力，还能够过滤掉低效的二字词。
...
但是，由于许多人不习惯使用大写字母，所以延顶支持在小写四码后用分号来将第四码变换为大写，显示的结果与上图完全一样。这种方式在避免大写的同时还带来了额外的好处，因为在加分号前，还可以进行各种组合变换操作。下图是加分号前的显示结果。

![image-20250213065624449](/images/image-20250213065624449.png)

如果想直接上屏「点张」，使用空格即可。后接分号和单引号以外的标点符号也会顶「点张」上屏。用Tab键可上屏两个二简词的组合即「代表这个」，这是延顶模式下特有的。另外，也支持与词组模式下一样的组合变换：用单引号上屏二简字和二简词的组合即「点这个」，用Shift+空格上屏二简字和两个一简字的组合即「点在个」。

要注意的是，延顶模式下三个声母后接分号的变换与词组模式下是不一样的。延顶模式下为输入二简字和标点字，词组模式下为输入一简字和二简字。例如，yxz;在延顶模式下输入的是「要最」，而在词组模式下输入的是「一息」。
