...
如果想直接上屏「点张」，使用空格即可。后接分号和单引号以外的标点符号也会顶「点张」上屏。

用Tab键在ssss格式时可上屏两个二简词的组合即「代表这个」，在sssb格式时上屏一个二简字和一个声笔分号字。用单引号在sssb格式时上屏一个二简字和一个声笔单引号字。这是延顶模式下特有的。

另外，也支持与词组模式下一样的组合变换：用单引号上屏二简字和二简词的组合即「点这个」，用Shift+空格上屏二简字和两个一简字的组合即「点在个」。
