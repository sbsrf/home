...
当二字词中的首字不是一、二简字的时候，在输入四码及以上后若发现首字不对，可以用Ctrl+L定位到第二码后进行快改，但是操作步骤仍然太多。有没有更便捷的方法呢？有的。那就是用大写的AEUIO追加首字的第三码，若有必要还可追加它的第四码。在追加第四码时可以不再用大写。当然，在第二字也不是一、二简字时，应该先追加第二字的编码，然后再追加第一字的编码。
...
为了缩短词组码长和增加输入确定性，提高输入效率，特为飞码提供了用数字选择的字词，可以在增强模式下使用。
