...
除了单字模式外，飞码还提供了延顶模式。延顶模式也是一种以单字为主的打字模式，它在兼容飞单的基础上，通过延迟顶屏克服了单字模式存在的一些问题，使得在输入词组时更加自然，具有词组模式下的组合变换能力，能够过滤掉低效的二字词。

比如，在输入编码dbzg的时候，结果如下图所示。

![image-20250123091055898](/images/image-20250123091055898.png)

继续输入编码会顶「点」字上屏，并留下zg和刚输入的编码。用空格可上屏两个二简字的组合即「点张」，用单引号可上屏二简字和二简词的组合即「点这个」，用Shift+空格可上屏二简字和两个一简字的组合即「点在个」。如果用大写AEUIO，则选中相应的候选项，但不会直接上屏，以便输入扩展编码。这时，要想直接上屏次选的「单字」，则可用分号。
...
过滤强度可以设置的值有3、4、5或6。如果设置为3，那么就不会进行过滤，因为二字词的总码长不可能小于四。如果过滤强度设置为5，二简字的码长为22、23、32这三种组合时就会被过滤掉。如果过滤强度为6，二简字的码长组合为33（但不含24和42）的也会被过滤掉。以下两图就分别是过滤强度设置为5和6时输入编码`dbzg`的结果。

![image-20250123093332042](/images/image-20250123093332042.png)

![image-20250123093400783](/images/image-20250123093400783.png)
...
![image-20250123093538969](/images/image-20250123093538969.png)

为了更便捷地选择高频的重码项，可以设置用Tab来进行三选，方法是修改飞码的schema文件，将下图最后一行前的`#`去掉，并重新部署。不过，这时就不能在四码时用Tab来翻页了，只能通过扩展编码来输入非首页的重码词，或者可以通过直接用`PageDown`（上机上是下滑Tab）来向下翻页。`Shift+Tab`（上机上是上滑Tab）的向上翻页功能仍然有效，仍可用于定位到最后一页来快速找到新造的词组。

![image-20250123093653818](/images/image-20250123093653818.png)
