...
为了更便捷地选择高频的重码项，可以设置用Tab来进行三选，方法是修改飞码的schema文件，将下图最后一行前的`#`去掉，并重新部署。不过，这时就不能在四码时用Tab来翻页了，只能通过扩展编码来输入非首页的重码词，或者可以通过直接用`PageDown`（手机上是下滑Tab）来向下翻页。`Shift+Tab`（手机上是上滑Tab）的向上翻页功能仍然有效，仍可用于定位到最后一页来快速找到新造的词组。

![image-20250123093653818](/images/image-20250123093653818.png)

还可以在有2位编码时按F6或Shift+空格快速在延顶、单字和词组模式之间切换。如果要设置延顶为默认模式，那么就需要修改sbfm.schema.yaml，并重新部署。
