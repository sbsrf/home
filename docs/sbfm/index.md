...
不过，即使对于一、二简字，仍然可以想其它办法来节约空格。在后面会讲到如何通过组合变换来实现这一点。
...
需要注意的是，一简字在组词的时候要输入其第二码，如前面例子中的`不bh 下xb 没md`，而声笔字的第二码不变，如前面例子中的`大de 心xo 见ji 全qu`。
...
![image-20240517105508643](/images/image-20240517105508643.png)
...
(9) 二简字加两个一简字的组合，可以用Shift+空格来输入，手机上可上滑空格。例如，「有一个」「这个人」「那不是」「走下去」等都可以这样输入。
...
飞码默认为词组模式。这种模式，从顺畅性和码长的角度来说，有利于输入大多数现代文，适用于大多数人的应用场景，因为现代文是以词组特别是二字词为主的。但是，不好的地方是，二简字在单独使用时就需要空格上屏了。还有一个要求，那就是要分词后输入，这对有的人来说可能会成为一个难点。不过，在想打的时候，对一般的人来说，都是以词为单位打腹稿的，所以分词输入是一个很自然的过程。
...
另外，在飞码的常规状态下，数字也可用来顶屏，因为这时飞码并不用数字来编码。
...
声笔飞码词组的四码空间容量与传统四码输入法如五笔字型是差不多的。但是，由于飞码收词量超过了100万，虽然飞码的离散很好，然而在个别情况下重码现象也会比较严重，可能需要多次翻页查找。于是，飞码采用了扩展编码，通过追加首字的前两个笔画，迅速扩大编码空间为四码时的25倍，可以快速筛选出所需的重码词组。结合后述动态码长的使用，完美地兼顾了编码空间与输入效率，既能享有巨大的编码空间，又能具有较短的平均码长，两全其美。
...
飞码的造词是在输入过程中自动进行的，无需用户干预。例如：输入`飞fa_ 码msaa`后，下次输入`fams`时就会有该词了，只是如果有重码的话，新造的词会排在后面。如果重码太多，可以追加首字前两笔或者用Shift+Tab一次性地跳到最后，以避免多次翻页。自动造的词组是临时词组，带有太极标志，优先级别低，只有在选择使用一次以后，才会成为正式词组，并提高优先级别。
...
除了单字模式外，飞码还提供了延顶模式。延顶模式也是一种以单字为主的打字模式，它通过延迟顶屏克服了单字模式存在的一些问题，使得在输入词组时更加自然，支持标点顶首选词，具有词组模式下的组合变换能力，能够过滤掉低效的二字词。但是，延顶也使得单字的输入变得不直观，第四码时选择重码也需换用大写的AEUIO，不过稍加熟悉就习惯了。因此，对于熟练的用户，在主单的时候，推荐使用更加高效的延顶模式。

比如，「单字」在输入编码dbzg的时候，完全按照词组模式的方式进行，无需使用大写字母，如下图所示。

![image-20250117083204605](/images/image-20250117083204605.png)

这时，要上屏首选的「单字」就不能用后续字词的首码来顶，而需用空格选择。不过，在后接标点符号如逗号、句号时，仍能直接将「单字」顶上屏幕，因此具有部分词组模式的优点。同时，要选择「单座」也需用大写的U，而不是小写的u。用分号可上屏两个二简字的组合即「点张」，用单引号可上屏二简字和二简词的组合即「点这个」，用Shift+空格可上屏二简字和两个一简字的组合即「点在个」。如果想使用扩展编码，那么得用大写AEUIO来输入第五码。

为了减轻记忆的压力，在首选词后用注释的方式提示出了两个二简字的组合。若要上屏前两码db对应的「点」字，那么直接用后续的编码顶屏即可，这是延顶模式下二简字的常规方式。后续的编码可以是第二字的第三码，也可以是后续字词的首码。
...
过滤强度可以设置的值有3、4、5或6。如果设置为3，那么就不会进行过滤，因为二字词的总码长不可能小于四。如果过滤强度设置为5，二简字的码长为22、23、32这三种组合时就会被过滤掉。如果过滤强度为6，二简字的码长组合为33（但不含24和42）的也会被过滤掉。以下两图就分别是过滤强度设置为5和6时输入编码`dbzg`的结果。注意，如果一个字有sbb缩减码，那么它的码长也算作3；比如，「码」的常规码为msaa，而缩减码为meu，因此也算3码字。

![image-20250115105537649](/images/image-20250115105537649.png)

![image-20250117085302352](/images/image-20250117085302352.png)

与词组模式下类似，如果进行过翻页，那么用小写的aeuio会直接选择重码上屏，而不是顶前两码对应的二简字上屏，这有利于重码的快速选择；同时，输入后续字词的声母也会就首选词顶上屏幕，而不必再用空格上屏。

![image-20250115111644083](/images/image-20250115111644083.png)

为了更便捷地选择高频的重码项，可以设置用Tab来进行次选，方法是修改飞码的schema文件，将下图最后一行前的`#`去掉，并重新部署。不过，这时就不能在四码时用Tab来翻页了，只能通过扩展编码来输入非首页的重码词，或者可以通过直接用`PageDown`（上机上是下滑Tab）来向下翻页。`Shift+Tab`（上机上是上滑Tab）的向上翻页功能仍然有效，仍可用于定位到最后一页来快速找到新造的词组。

![image-20250121185010252](/images/image-20250121185010252.png)

还可以在有2位编码时按F6或Shift+空格快速在延顶、单字和词组模式之间切换。如果要设置延顶为默认模式，那么就需要修改sbfm.schema.yaml，并重新部署。注意，这个设置也会同时对飞码的词组模式生效。
