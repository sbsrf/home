...
除了单字模式外，飞码还提供了延顶模式。延顶模式也是一种以单字为主的打字模式，它通过延迟顶屏克服了单字模式存在的一些问题，使得在输入词组时更加自然，支持标点顶首选词，具有词组模式下的组合变换能力，能够过滤掉低效的二字词。但是，延顶也使得单字的输入变得不直观，第四码时选择重码也需换用大写的AEUIO。对于熟练的用户，在主单的时候，推荐使用更加高效的延顶模式。
