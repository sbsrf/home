...
在默认的情况下，声笔自然的单字是从第四码开始调频的，这有利于三简字的盲打，但是不利于缩短单字的码长。如果用户应用场景比较简单和单一，那么可能希望从第三码开始单字调频，以便尽量缩短码长。这时，可以按F4选择2后，在菜单中将慢调改为快调。若要默认为快调，可以修改schema文件，将switches中fast_change项下的reset设置为1，并重新部署一下。注意，快调只影响单字，不会影响词组。
