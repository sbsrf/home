...
例如，「的」的编码为d，只需要声母，是一级简码；「使」的编码为sr，只需要声母s（不分平翘舌）和部首r（人旁「亻」读音的声母），是二级简码；「编」的编码为bjo，由声母b、部首j（绞丝旁「纟」读音的声母）和剩余部分「扁」的首笔点o构成，是三级简码；「码」的编码为msaa，由声母m、部首s（「石」字读音的声母，不分平翘舌）和剩余部分「马」的前两笔折折aa构成，是全码。
...
飞单有105个特殊的声笔字，采用声母加首笔编码。声笔字的目的是兼容声笔简系的这105个字，同时提高编码的离散，还能支持声笔飞讯的主单模式，具有非常重要的意义。

例如，「为」的编码为声母w加首笔点o，而「进」的编码为声母j加首笔横e。

### 2.4 取部规则

编码时取的部首与字典的部首类似，可能在前，也可能在后。为什么可前可后呢？因为这样可以提高编码区分重码的能力。例如，「墓募慕蓦暮謩摹」这些字如果均取「艹」作为部首就会产生大量重码，而取「莫」以下的部分作为部首则可以在前两码就有效地区分重码，同时也与语文知识相符合。但是，这么做给学习过五笔之类顺序拆分法的飞单初学者带来了一定的困难，然而这是为了获得强大离散能力必须作出的牺牲。

不过，只要掌握了一般的取部规则，举一反三，绝大多数困难都会迎刃而解。**这个规则是：**<u>将汉字尽量直观地拆分为左右或者上下两个部分，再按先后顺序取部首，前面部分优先，前面部分不是部首就取后面部分，但是单笔画的优先级别更低；若前后两部分都不是部首，则继续按上述原则拆分。若偏旁部首本身是一个成字，那么就取它的第一个笔画作为部首。注意，为了尽量离散重码，存在个别例外，会在特取字中列出。</u>

具体说来，

- 飞单的部首一般在字的首部或尾部，将汉字直观地拆分为两个部分，如果有一个部分是部首，就取这个部分。
  例如，「制」分成制的左半和立刀旁，制的左半不是部首，则取立刀旁。
  
- 如果两个部分都是部首，则取第一部分。
  例如，「红」分成绞丝旁和工，取第一部分绞丝旁。
  
- 如果两个部分都不是部首，则继续把两个部分拆分成四个部分，再取首部或尾部。
...
- 还有部分部首，它们的取部规则和非部首字一样：「广」取「厂」，编码为gc；「走」取「土」，编码为zt；「麦」取「夂」，编码为mwe；「龍」取「立」，编码为llia。
...
![image-20240304122918423](/images/image-20240304122918423.png)

对于零声母字，如「案」字，在反查时除了用i引导还需要在正常拼音的前面加v，因为an是零声母音，所以实际的输入为avan。在图中提示出的`案vm【木】`，表示「案」字的编码为vm，其部首为「木」。

![image-20240304123008992](/images/image-20240304123008992.png)
...
![image-20240304123128054](/images/image-20240304123128054.png)

![image-20240304123152238](/images/image-20240304123152238.png)

对于非GBK字集里的「㕛」字，它是没有编码的，只能用两分法来输入，如下所示。此时，需要移动光标选中该字，然后再用空格上屏。

![image-20240304123240872](/images/image-20240304123240872.png)
...
![image-20240316093630618](/images/image-20240316093630618.png)
...
部首的命名和代码是学习的关键之一，能够深刻理解的话，对初学者帮助很大，本文附录中给出一个完整的[部首列表](#附录-部首列表)，包含其序号、代码、名称和例字。

### 3.2 编码规则

飞单中，一共收录了GBK的2万多字，其编码规则为声母 + 部首 + 去掉部首后剩余部分的前两个笔画。需要注意的是，如果一个字本身为部首，那么其编码方式特殊，涉及的问题比较复杂，将专门在[部首编码](#_2-2-6-部首编码)一节详细讲解。还要注意的是，「戴」「威」「咸」之类的字在取部首「戈」后，笔画需从首笔开始，因为按照笔顺写字的时候「戈」会被拆开，不是连续写出来的。
...
在待输的字不是简码字，而且输入的编码正确时，如果本页没有所需的字，则用`Tab`键翻页查找，而`Shift+Tab`则用于反向翻页。如果通过翻页还是找不到所需的字，则该字肯定不在GBK字集中，是没有编码的，此时只能用[两分反查](#_2-3-4-两分反查)来输入该字。
...
在[偏旁部首](#_2-1-2-偏旁部首)一节，介绍了部首的类型、助记和位置，知道部首分为成字部首和非成字部首。本节对部首进行细分，并讨论它们的编码，这是学习飞单的一个难点和重点。成字部首细分为常规部首、声笔部首、变形部首和特殊部首。非成字部首细分为易显部首和难显部首。
...
飞单实际上只是[飞码](../sbfm/)的单字模式而已，所以它可以通过大写第三码来支持常规的飞码词组。这在输入那些不知道或者想不起部首的复杂成语时非常有用，也可以用来缩短码长而提高输入速度，例如：「魑魅魍魉cmWl」「风驰电掣fcDc」「螳螂tcLc」「蜿蜒wcYc」等，不过显示出来的编码仍然是全部小写的。
...
![image-20240422091102772](/images/image-20240422091102772.png)
...
![image-20240422091603273](/images/image-20240422091603273.png)

![image-20240422091619111](/images/image-20240422091619111.png)
...
(3) 声偏字：441个，详见[声偏字](#_2-2-2-2-声偏字)
...
如果您还想输入大量的一般词组，那么在基本上熟习飞单后，即使还不能够完全盲打，就可以继续学习声笔飞码或者声笔飞讯。也可以将飞单与简码增强模式结合使用，即使用简码的快单增强模式，又称声笔飞简。下表列出了四者的主要特点，以便您进行合理的选择。
...
| 声笔飞简 | 二三混顶，无延迟，动静结合 | 简码字词，数选字词，动静结合 |
