...
在词组模式下，飞简单字输入与在单字模式的唯一区别是，声偏字需要空格上屏。例如，「声」「偏」需要用`st_pr`来输入，而不能用`stpr`，「偏」的声母p并不能将「声」顶上屏幕。当然，用大写「偏」的首码仍然将「声」顶上屏幕。
...
如果不想在输入基本编码时就选择重码，而只想在输完扩展编码后才选择，那么可以将single_selection设置为true。这时，会在输入的时候，对后续的词组编码进行提示。如果觉得这种提示对你有干扰，可将Translator下的single_display选项设置为1，这样就只有首选才提示了。若遇到不知道后续编码的情况，可在输入四码及以上时按Tab临时打开后续编码提示。
...
为了支持更丰富的组合上屏，以便兼容词组模式或者节约用键，飞简提供了延顶模式。在延顶模式下，输入四个声母（如jsjd）时，显示的是空码，不要慌张，其结果会因后续的上屏键而异，分为以下几种情况。

- 三码词和一简字的组合：用空格上。若jsj对应的是「计算机」，那么jsjd会上屏「计算机的」。
- 三码词和二简字的组合：后接小写字母时顶屏。若jsj对应的是「计算机」，那么jsjda会上屏「计算机」并留下声笔字「对da」，而jsjdl会上屏「计算机」并留下声偏字「到dl」。
- 声偏字和三码字词的组合：后接大写字母时顶屏，jsjdA会上屏「尽」并留下三码字「决jda」，而jsjdL会上屏「尽」并留下三码词「激烈jdl」，假设jdl对应的是「激烈」。
- 两个声偏字的组合：用分号上屏，可节约一次空格，jsjd会上屏「尽江」。
- 声偏字和二简词的组合：用单引号上屏，可节约一次空格，jsjd会上屏「尽觉得」。
- 两个二简词的组合：用Tab，可节约一次Tab，jsjd会上屏「建设觉得」。

- 二简字和数选词的组合：顶屏，节约一次空格，jsjd3会上屏「尽见到」。
