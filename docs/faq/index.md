...
声笔在输入半角字符是有着比较特殊的设计。一般的输入方案只能在西文状态下输入半角字符，而声笔可以在中文状态下进行输入，其原理是延迟上屏。也就是说，在输入相应的字符后，这个字符并不是作为中文字符马上上屏，而是列出可能的标点符号供用户选择，比如在使用声笔简拼时输入`/`后的提示信息为如下所示。

![image-20240822071029008](/images/image-20240822071029008.png)

令人迷惑的是，这时并没有显示半角的符号作为选项之一，但是你可以回车就输入半角的`/`了。为什么不提示出来呢？因为提示出来就占用了其他符号的位置，显得多此一举、杂乱无章，因为回车输入半角就是一个简单的规则，用户知道后就能如法炮制输入所有半角符号了。