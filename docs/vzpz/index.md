...
- 将 `sbsrf.zip` 压缩包中的所有内容复制到用户文件夹 rime 中。由于访问权限的原因，小企鹅本身的文件管理功能，具体操作方法为：
  - 小企鹅输入法→高级→浏览用户目录；
  - 通过左上角图标进入菜单，选取你的手机，浏览到你下载的sbsrf.zip文件；
  - 如果是在QQ群下载的，那么路径一般为/Android/data/com.tencent.mobileqq/Tencent/QQfile_recv；
  - 直接打开sbsrf.zip文件，通过右上角菜单全选其中的所有文件和子文件夹；
  - 通过右上角菜单，选择「解压到...」；
  - 通过左上角菜单，选择「小企鹅输入法」，进入data文件夹，再进入rime文件夹；
  - 若屏幕下边出现的是「复制」按钮，说明已经有旧的声笔方案，请先删除除userdb而外的所有文件；
  - 否则屏幕下边应该是「解压」按钮，点击执行并等待完成；
  - 打开一个应用（比如QQ），在弹出的键盘上，点击右上角的`...`图标，再点击「重载配置」；
  - 再点击`<>`图标选择你想用的声笔方案，就可以享用了。
