...
- 如果已经安装过小狼毫，请先退出算法服务，打开用户文件夹 rime，备份其中各方案的 userdb 目录，然后删除整个 rime 目录。
...
- 如果备份了 userdb 目录，再将它们复制回 用户文件夹rime 中。
...
![ABB252F69E0F2CDD6F931C393294B435](/images/ABB252F69E0F2CDD6F931C393294B435.jpg)

- 通过键盘选项可以自行根据需要进行一些设置，如按键音、空格键行为、键盘高度等。
- 通过主题选项可以选择主题，并进行相关配置。
- 将 `sbsrf.zip` 压缩包中的所有内容（注意不要包含sbsrf目录本身）复制到用户文件夹 rime 中，该文件夹的路径为内部存储/Android/data/org.fcitx.fcitx5.android/files/data/rime。由于访问权限的原因，第三方文件管理器往往不能查看这个文件夹，所以需要用手机自带的文件管理器或者小企鹅本身的文件管理功能。
- 如果备份了 userdb 目录，再将它们复制回用户文件夹 rime 中。
