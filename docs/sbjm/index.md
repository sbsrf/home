...
再如， 想打「加快进度」，在输入jkjd后，发现为空码，系统用两个二简词的组合进行了填充，提示是这样的：

![image-20240401091457816](/images/image-20240401091457816.png)

接着输入「进度」的第三码o，顶出前两码jk对应的二简词「加快」后，留下了jdo，正好是「进度」的前三码。
...
声笔简码的默认模式为四顶常规模式，根据使用场景和用户的使用习惯不同，可能需要对模式进行调整。在电脑上可以用Ctrl+~或F4，在手机上直接点击状态栏的相应选项，就可以切换模式了。如果要将某种模式设置为默认状态，只要修改sbjm.schema.yaml文件中相应状态开关项的reset值，并重新部署一下就可以了。
...
注意上图红框中还有个参数lower_case。当它的值为false时，在三顶模式下，若采用一二三末字的声母来输入多字词，则第四码必须用大写字母。例如，「爱因斯坦」就需要用`vysT`来输入才行。
...
例如，在输入j后，当提示级别分别为有理、无理、两者和隐藏时，数选字的提示情况如下所示。

![image-20240511073801161](/images/image-20240511073801161.png)

![image-20240511073828124](/images/image-20240511073828124.png)

![image-20240511073907284](/images/image-20240511073907284.png)
...
另外，若提示级别不为隐藏，那么在增强模式下会通过注释的方式提示数选字词。

![image-20240505084728291](/images/image-20240505084728291.png)

![image-20240505084750700](/images/image-20240505084750700.png)

需要注意的是，为了充分利用码位，在声笔数字的有理组中，也可能会出现数字不表示笔画的情况，这是在有理的空间中无理地安排了单字。

例如，输入vi后的结果如下。其中，「唉哦昂哎」都不是用有理数字来选择的。

![image-20240511135144740](/images/image-20240511135144740.png)

#### 4.2.2 标选单字

细心的读者可能已经注意到，在上节讲解数选字的例子中，输入j后，在首选字「就」后面跟着「建; 解'」作为它的注释，表示「建」可以用分号选择，而「解」可以用单引号选择。另外，在输入前两码后，有可以用分号和单引号来选择单字。它们就是用标点选择的单字，称为标选单字。

例如，输入w后，提示的标选单字为「往; 完‘」，如下所示。

![image-20240511075806993](/images/image-20240511075806993.png)

再如，输入提示的标选单字为「嗡; 巍‘」，如下所示。

![image-20240511141545983](/images/image-20240511141545983.png)

### 4.3 兼容飞单

为了增加单字的离散能力，声笔简码还兼容了声笔飞单里的单字。声笔飞单的一简字和声笔字与声笔简码完全相同，它的二、三简字则作为首选的注释用单引号或者Tab键选择，其它的单字通过将第四码aeuio置换为23789或者在第四码后追加单引号来输入，但是实际输入的编码始终是四码加单引号，以下是示例。

![image-20240410082728469](/images/image-20240410082728469.png)

![image-20240411081214094](/images/image-20240411081214094.png)

![image-20240411080827112](/images/image-20240411080827112.png)

实际上，即使不在增强模式下，单字的飞单编码格式也是随时可用的，只是没有提示而已，以免分散初学者的注意力。另外，若提示级别为隐藏，那么在增强模式下也不会通过首选注释的方式提示飞单的。

### 4.4 声声偏词

声笔简码通过兼容飞单解决了单字离散太弱的问题，然而二字词离散太弱的问题仍然存在。二字词在现代汉语中占据主导地位，对输入效率影响很大。如何解决这个问题呢？这个问题的症结在于ssb的空间太小，只有`21*21*5 = 2205`个码位。所以，解决问题的出路在于如何增加扩大编码空间。如果能够进一步结合飞单来为二字词编码，从而利用其他的三码空间，那么就可以大大缓解二字词的离散压力。

最终想到的解决方案是，增加s1s2p2b2b1b1方式编码的二字词，适用于末字为声偏字的二字词，在打开schema中translator项下的enable_ssp_words子项时启用，一般需要结合三顶模式来使用，否则在三码时需要空格上屏，对效率的提升作用不大。

例如，「编辑」和「区位」既可以用ssp格式，也可以用ssb格式来输入。但是，「故而」和「壮大」只能用ssb格式输入，因为「而」是一简字，而「大」是声笔字。

### 4.5 简飞模式

为了让声笔简码取得尽量高的输入效率，可以采用简飞模式，亦即将以下涉及到的选项设置如下。
...
translator:
  ...
  lower_case: false
  enable_ssp_words: true

```

其中，将三顶third_pop设为1保证字词均可三码起顶，增强is_enhanced为设1保证可以使用数选字词和标选单字，单次选重single_selection设为true保证可以在非全码时只观察首选即可，配合单项提示single_display为true可以避免过多的提示信息，启用过滤enable_filtering设为false可以在三码时出现多字词，小写lower_case设为false强制多字词只能用ssss格式来输入以便将更多的sszb空间让给二字词和三字词，启用声声偏词enable_ssp_words设为true可以大大增强二字词的离散能力。
