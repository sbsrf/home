...
最终想到的解决方案是，增加s1s2p2b2b1b1方式编码的二字词，适用于末字为声偏字的二字词，在打开schema中translator项下的enable_ssp_words子项时启用，一般需要结合三顶模式来使用，否则在三码时需要空格上屏，对效率的提升作用不大。这时，ssp二字词就不能用ssb格式来输入了。
