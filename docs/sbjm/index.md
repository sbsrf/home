...
声笔简码极其简单，没有什么学习门坎，可以用一句话概括它的内容：用v表示零声母，用`aeuio`分别表示折、横、撇、竖、点五种笔画，采用简拼加笔画编码。笔画不足时，则重复末笔。一旦使用笔画或者满四码后，就可以省略空格。
...
其中，s表示声母（不区分平翘舌），b表示笔画。b为元音字母`aeuio`，s为剩下的21个辅音字母。
...
对于非成字偏旁部首，可以用纯笔画的方式输入。比如，「亻」可以用`uiiii`输入。
...
标点词组均为二字词，其编码为首字声母加用`,;/.'`表示的首字首笔，再加次字首笔的有210个词，再加次字前两笔的有1039个词。其中，`;`表示竖，`'`表示折。

![image-20260624160259015](/images/image-20260624160259015.png)

![image-20260624160334695](/images/image-20260624160334695.png)
...
数选字词包括声数字210个，声笔数字1050个，声声数词4410个，它们均分为23789选择的有理组和14560选择的无理组。

根据提示级别的不同，可以只提示有理组，只提示无理组，两组同时提示，或者两组都不提示。系统默认设置为只提示有理组。提示与否仅仅是视觉上的差异，只要开启了增强模式，这些数选字词始终是可以使用的。
...
为了便于互击操作，在输入声声数词时，1可以用单引号来代替，而0可以用Tab来代替。例如，「计算」可以用`js1`或者`js'`来输入，而「结束」可以用`js0`或者`js+Tab`来输入。

![image-20250623092339031](/images/image-20250623092339031.png)
