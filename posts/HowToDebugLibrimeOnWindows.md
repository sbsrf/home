...
采用默认的release版生成调试信息（/DEBUG）时，许多调试信息都被优化掉了，在逐行调试时，很多变量都跟踪不了，有的语句还会被跳过。为了解决这个问题，可以将rime项目属性的C/C++项中的优化子项里的优化通过下拉菜单由/O2改为/Od，内联函数扩展通过下拉菜单由/Ob2改为/Ob0，然后重新生成rime项目。
