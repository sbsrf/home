...
1. 如果拼音部分是 3 码，则补前 2 个笔画，格式简记为 syybb；
2. 如果拼音部分是 2 码，则补前 3 个笔画，格式简记为 sybbb。

为了在下文中方便描述，我们将这两种格式统称为 syxbb，x 代表第三码可能为 y 也可能为 b。

本方案中，单字为二码顶，也即两码及以上的单字都不需要空格上屏。

## 词语输入

### 二、三、四字词的输入

- 二字词的基本编码为 4 码 ssyx，即第一个字的首码加上第二个字的前三码；
- 三字词的基本编码为 5 码 sssyx，即前两个字的首码加上第三个字的前三码；
- 四字词的基本编码为 6 码 ssssyx，即前三个字的首码加上第四个字的前三码；

总而言之，前面的字都只需要首码，最后一个字需要三码。本方案中，二字词为三码顶、三字词和四字词为四码顶。

### 多字词的输入

多字词的基本编码为 ssssys...syx，也就是说 n 字词的基本编码为 n+3 码。注意，因为需要支持四字词的四码顶，所以更长的词不能直接输入后面的字的首码，而是需要输入一位韵母之后，才能继续输入后面的声母。

### 扩展编码

由于词汇的数量很多，只输入基本编码的时候可能产生超过一页的重码（特别是二字词），这时候可以继续补充第一个字的编码来化解重码。当最后一个字已经输入了 3 码时，再输入 aeiou 中的按键时，这些编码会自动追加到第一个字后面，不需要手动定位来补码。

扩展编码最多可以输入 3 码，即将第一个字补充至最多 4 码。
...
固顶字词在一般的连续性输入文本中的总频率已经达到了 60% 以上，所以掌握固顶字词可以快速提高输入方案的熟练度。

### 简码后置

为了降低重码，在输入的过程中，如果一个字词已经在首选出现过，那么在后续输入中它将不再位于首选。
...
这个表格主要是为了让大家理解三拼对零声母的处理方式，需要注重观察规律，不要死记硬背。
